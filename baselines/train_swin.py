...
LOG_FILE      = os.path.join(RESULTS_DIR, "baseline_swin_log.csv")
...
def main():
    ...
    MODEL_SAVE_PATH = f"{CHECKPOINT_DIR}/best_swin_fold{FOLD}.pth"
    checkpoint_path = f"{CHECKPOINT_DIR}/latest_swin_fold{FOLD}.pth"

    print(f"\n{'='*50}")
    print(f"  SWIN TRANSFORMER BASELINE | FOLD {FOLD} | {IMG_SIZE}×{IMG_SIZE} | {NUM_CLASSES} classes")
    ...
    print(f"\n✅ Done. Best Swin Val AUC={best_val_auc:.4f} | F1={best_val_f1:.4f}")
